...
class TestUtil(unittest.TestCase):
    ...
    def test_find_segment(self):
        ...
        self.assertEqual(myutils.find_segment(5.5, test_list), 5)
        self.assertEqual(myutils.find_segment(0, test_list), 0)
        self.assertEqual(myutils.find_segment(5, test_list), 5)

        # TODO: fix this test case ()
        # assert myutils.find_segment(9, test_list) == 8

        # TODO: implement extrapolation (check if it works with interp.)
        self.assertEqual(myutils.find_segment(-10, test_list), 0)
        self.assertEqual(myutils.find_segment(20, test_list), 8)

    def test_linear_spline(self):
        x_test_list = np.arange(10)
        y_test_list = x_test_list + 1

        self.assertEqual(myutils.linear_spline(0, x_test_list, y_test_list), 1)
        self.assertEqual(myutils.linear_spline(7, x_test_list, y_test_list), 8)
        self.assertEqual(myutils.linear_spline(3.1234, x_test_list, y_test_list), 4.1234)
        self.assertEqual(myutils.linear_spline(9, x_test_list, y_test_list), 10)

        # Test extrapolation ?
        self.assertEqual(myutils.linear_spline(20, x_test_list, y_test_list), 21)
        self.assertEqual(myutils.linear_spline(-30, x_test_list, y_test_list), -29)
